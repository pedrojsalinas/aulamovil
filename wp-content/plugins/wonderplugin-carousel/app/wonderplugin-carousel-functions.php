...
function wonderplugin_carousel_tags_allow( $allowedposttags ) {
	...
	$allowedposttags = apply_filters( 'wonderplugin_carousel_custom_tags_allow', $allowedposttags );
...
function wonderplugin_carousel_css_allow($allowed_attr) {
	...
	$allowed_attr = apply_filters( 'wonderplugin_carousel_custom_css_allow', $allowed_attr );
...
function wonderplugin_carousel_dirtoarray($dir, $recursive) {
	...
	foreach ($cdir as $key => $value)
	{
		if (!in_array($value,array(".","..")))
		{
			if (is_dir($dir . DIRECTORY_SEPARATOR . $value))
			{
				if ($recursive)
				{
					$result[$value] = wonderplugin_carousel_dirtoarray($dir . DIRECTORY_SEPARATOR . $value, $recursive);
				}
				else
				{
					$result[] = $value;
